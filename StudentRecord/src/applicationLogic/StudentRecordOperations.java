...
/*	public LinkedList<Student> sortedRecordName(LinkedList<Student> Record)
	{
		...
		public class MyLinkedListSort {
		    ...
		    public static void main(String a[]){
		        ...
		        for(Student s:nameSortedRecord){
		...
		class nameComp implements Comparator<Student>{
		    ...
		    public int compare(Student s1, Student s2) {
		    	boolean bool = false;
		    	Student sTemp = new Student(null,null,null,0,0,0);
		    	for (int i = 0; (i -1 <= (s1.getName().length()) && (s1.getName().length())) && !(bool); i++) {
				
			
		    		if(s1.getName().charAt(i) > s2.getName().charAt(i)){         
		    			int j=(nameSortedRecord.indexOf(s1)+nameSortedRecord.indexOf(s2))/2;
		    			sTemp.equals(s2);
		    			nameSortedRecord.get(j+(1/2)).equals(s1);
		    			nameSortedRecord.get(j-(1/2)).equals(s2);
			    
		    		}    
		    		else (s1.getName().charAt(i) < s2.getName().charAt(i)){
		    			int j=(nameSortedRecord.indexOf(s1)+nameSortedRecord.indexOf(s2))/2;
				        sTemp.equals(s1);
				        nameSortedRecord.get(j+(1/2)).equals(s2);
					    nameSortedRecord.get(j-(1/2)).equals(s1);
